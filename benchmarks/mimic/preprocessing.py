...
def assemble_episodic_data(stays, diagnoses):
    ...
    # reorder columns
    data = data[['Ethnicity', 'Gender', 'Insurance', 'Age', 'Height', 'Weight', 'Length of Stay', 'Mortality']]
...
def extract_diagnosis_labels(diagnoses):
    ...
    diagnoses['VALUE'] = 1
    # create 1-0 matrix to indicate which icustay is associated with which ICD9 codes
    labels = diagnoses[['ICUSTAY_ID', 'ICD9_CODE', 'VALUE']].copy().drop_duplicates().pivot(index='ICUSTAY_ID', columns='ICD9_CODE', values='VALUE').fillna(0).astype(int)
    # add column of zeros for missing ICD9 codes
    for l in diagnosis_labels:
        if l not in labels:
            labels[l] = 0
    # reorder the columns 
    labels = labels[diagnosis_labels]
    # remane the columns (add keyword 'Diagnosis')
    return labels.rename(dict(zip(diagnosis_labels, ['Diagnosis ' + d for d in diagnosis_labels])), axis=1)


# definitions: use hcup_ccs_2015_definitions.yaml file to retain only 25 prespecified conditions
def add_hcup_ccs_2015_groups(diagnoses, definitions):
    # map definitions/diseases to their usage for benchmark
    def_map = {}
    for dx in definitions:
        for code in definitions[dx]['codes']:
            def_map[code] = (dx, definitions[dx]['use_in_benchmark'])
    # store the information about the HCUP definition and benchmark usage 
...
def make_phenotype_label_matrix(phenotypes, stays=None):
    # 1 considered TRUE (as non-zero) --> select all phenotypes used for benchmarking
    ...
    # generate matching phenotypes table for a the stay table
    if stays is not None:
        # phenotypes = phenotypes.iloc[stays.ICUSTAY_ID.sort_values()] 
        # could be changed to: 
...
# fn: itemid_to_variable_map.csv in resources directory
def read_itemid_to_variable_map(fn, variable_column='LEVEL2'):
    ...
    # all rows with status 'ready' get included
...
# ranges given by variable_ranges.csv in resources dir
...
# SBP (systolic blood pressaure): some are strings of type SBP/DBP
def clean_sbp(df):
    v = df.VALUE.astype(str).copy()
    # filter out the occurences containing / and select first group
...
# CRR(capillary refill rate): strings with brisk, <3 normal, delayed, or >3 abnormal --> transform to binary
def clean_crr(df):
    v = Series(np.zeros(df.shape[0]), index=df.index)
    v[:] = np.nan

    # when df.VALUE is empty, dtype can be float and comparison with string
...
# FIO2 (fraction inspired oxygen): many 0s, some 0<x<0.2 or 1<x<20
def clean_fio2(df):
    ...
    '''
    is_str = np.array(map(lambda x: type(x) == str, list(df.VALUE.copy())), dtype=np.bool)
    # check that UOM is not torr (unit of pressure) and it's either a string or above 1
    idx = df.VALUEUOM.copy().fillna('').apply(lambda s: 'torr' not in s.lower()) & (is_str | (~is_str & (v > 1.0)))
...
def clean_height(df):
    ...
    v.loc[idx] = np.round(v[idx] * 2.54) # inches to centimeters) 
    return v


### Unused variables
# ETCO2 (amount of CO2 in exhaled air): haven't found yet
...
### All variables (#17) used in model: no cleaning if commented
