...
def clean_charlson(stays, verbose=False):
    ...
    stays['comorb_90'].replace("None", 0, inplace=True)
    stays['comorb_180'].replace("None", 0, inplace=True)
    
    # convert strings to floats
    stays['comorb_90'] = stays['comorb_90'].apply(lambda g: int(g))
    stays['comorb_180'] = stays['comorb_180'].apply(lambda g: int(g))
