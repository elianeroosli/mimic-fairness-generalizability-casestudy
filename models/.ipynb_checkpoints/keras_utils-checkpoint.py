# +-------------------------------------------------------------------------------------------------+
# | keras_utils.py: metrics and layers for keras models                                             |
# |                                                                                                 |
...
class InHospitalMortalityMetrics(keras.callbacks.Callback):
    def __init__(self, train_data, val_data, train_dem, val_dem, target_repl, data_name, batch_size=32, early_stopping=True, verbose=2):
        super(InHospitalMortalityMetrics, self).__init__()
        self.train_data = train_data
        self.val_data = val_data
        self.train_dem = train_dem
        self.val_dem = val_dem
        ...
        self.train_history = []
        self.val_history = []
        self.date = utils.give_date()

    def calc_metrics(self, data, dems, history, dataset, logs, epoch):
        y_true = []
        predictions = []
        B = self.batch_size
        ...
        for dem in dems.columns:
            for val in dems[dem].unique():
                ...
                y_group = [val for i,val in enumerate(y_true) if idx[i]]
                ret, c, i = metrics.print_metrics_binary(y_group, pred_group, dem, val)
                metric_list.append(ret)
        
        ret, c, i = metrics.print_metrics_binary(y_true, predictions)
        ...
        # store metrics
        metric_list.append(ret)
        path = os.path.join('models/outputs', self.data_name, 'predictions/metrics/train', self.date, str(epoch) + ".csv")
        utils.save_metrics(metric_list, path)

    def on_epoch_end(self, epoch, logs={}):
        print("\n==>predicting on train")
        self.calc_metrics(self.train_data, self.train_dem, self.train_history, 'train', logs, epoch)
        print("\n==>predicting on validation")
        self.calc_metrics(self.val_data, self.val_dem, self.val_history, 'val', logs, epoch)
