...
import numpy as np
from models.mimic.evaluation import metrics
...
class InHospitalMortalityMetrics(keras.callbacks.Callback):
    def __init__(self, train_data, val_data, train_dem, val_dem, target_repl, batch_size=32, early_stopping=True, verbose=2):
    ...
    def calc_metrics(self, data, dems, history, dataset, logs, epoch):
        ...
        # store metrics
        metric_list.append(ret)
        path = os.path.join('mimic3models/in_hospital_mortality', 'train_predictions', self.date, str(epoch) + ".csv")
