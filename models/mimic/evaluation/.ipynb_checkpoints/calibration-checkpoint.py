...
def plot_calibration_mean(output_json):
    fig, axs = plt.subplots(5, 1, figsize=(9,6), sharex=True, gridspec_kw={'height_ratios': [1,2,3,2,4], 'hspace': 0})
    plt.xlabel("Average risk score", fontsize=16, labelpad=10)
    axs[1].margins(y=0.15)
    axs[2].margins(y=0.10)
    axs[3].margins(y=0.15)
    axs[4].margins(y=0.05)
    ...
    for idx, key in enumerate(output_json.keys()):
        ...
        for group in df.columns:
            ...
            for v, variable in enumerate(variables):
                ...
                axs[idx-1].hlines(y=labeldict[group]+str(v), xmin=med-std, xmax=med+std, linewidth=3, color=colors['cint'][v])
                axs[idx-1].scatter(y=labeldict[group]+str(v), x=med, color=colors['cint'][v])
                # invisible padding for plot
                if v==0:
                    axs[idx-1].scatter(y=labeldict[group],x=0.1, s=0)
                if v==1:
    ...
    fig.tight_layout()
    path = os.path.join('mimic3models/in_hospital_mortality/plots/calibration', "confval_calibration_plot.jpg")   
    fig.savefig(path)
...
def plot_calibration_diffs(output_json):
    fig, axs = plt.subplots(5, 1, figsize=(9,5), sharex=True, gridspec_kw={'height_ratios': [1,2,3,2,4], 'hspace': 0})
    ...
    fig.tight_layout()
    path = os.path.join('mimic3models/in_hospital_mortality/plots/calibration', "confval_calibration_diffs_plot.jpg")   
    fig.savefig(path)
...
def calibration_overall(df, name, g=10, option='exp_quantiles'):
    ...
    path = os.path.join('mimic3models/in_hospital_mortality/plots/calibration/individual', name + "_calibration_overall.jpg")   
    fig.savefig(path)
