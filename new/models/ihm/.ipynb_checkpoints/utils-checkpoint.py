...
def load_data(reader, discretizer, normalizer, small_part=False, return_names=False):
    ...
    if reader._data_name == 'starr':
        whole_data = (np.array(data), labels, comorb1, comorb2)
    else:
...
def save_results_comorb(names, pred, y_true, comorb1, comorb2, dems, path):
    common_utils.create_directory(os.path.dirname(path))
    with open(path, 'w') as f:
        f.write("stay,prediction,y_true,comorb1,comorb2,Insurance,Gender,Ethnicity\n")
        for (name, x, y, c1, c2, dem) in zip(names, pred, y_true, comorb1, comorb2, dems.values.tolist()):
            f.write("{},{:.6f},{},{},{},{},{},{}\n".format(name, x, y, c1, c2, dem[0], dem[1], dem[2]))
