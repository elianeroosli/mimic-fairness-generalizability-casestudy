# +-------------------------------------------------------------------------------------------------+
# | common_utils.py: diverse helper functions                                                       |
# |                                                                                                 |
...
from models.resources.functionalities.feature_extractor import extract_features


def optimal_epoch(log_name, data_name):
    path = os.path.join('models/outputs', data_name, 'keras_logs', log_name)
    keras_log = pd.read_csv(path, sep=';')
    print(np.argmax(keras_log['val_auroc'])+1)
    

def convert_to_dict(data, header, channel_info):
    """ convert data from readers output in to array of arrays format """
    # initialize container with correct number of []'s
...
def read_chunk(reader, chunk_size):
    ...
    for i in range(chunk_size):
        # get back two dictionaries: store information in dicts that store info over all samples
        ret, dems = reader.read_next()
        for k, v in ret.items():
            if k not in data:
                data[k] = []
            data[k].append(v)
        for k, v in dems.items():
            if k not in demographics:
                demographics[k] = []
            demographics[k].append(v)  
    
    # need only first entry for header as it is the same for each sample
    data["header"] = data["header"][0]
    demographics["header"] = demographics["header"][0]
