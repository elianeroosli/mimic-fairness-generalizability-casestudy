...
# group samples into risk groups based on the prediction and compute mean predicted and observed mortality risk per group
def risk_groups_percentage(df, g):
    df.reset_index(drop=True, inplace=True)
...
def risk_groups(df, g):
    df.reset_index(drop=True, inplace=True)
    #dev = np.ceil(g*0.2)
    #perc_lower = 100/(g+dev)
    #perc_upper = 100/(g-dev)
    #df['perc_size'] = [perc_lower if r<0.5 else perc_upper for r in df['prediction']]
    #[0, 0.25, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.98, 1]
    #-(np.geomspace(2,1,12)-2)
...
def calibration_overall(df, name):
    ...
    sm.proportion.proportion_confint(count, nobs, alpha=0.05, method='wilson')
