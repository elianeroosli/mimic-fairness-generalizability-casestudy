...
def make_phenotype_label_matrix(phenotypes, stays=None):
    ...
    if stays is not None:
        ...
        # could be changed to: phenotypes = phenotypes.reindex([stays.ICUSTAY_ID.sort_values()])
...
########## SHOULDN'T IT BE f INSTEAD OF F?????
...
def clean_height(df):
    ...
    v.ix[idx] = np.round(v[idx] * 2.54) # inches to centimeters)
    return v
