...
def main():
    ...
    parser.add_argument('--output_dir', type=str, help='Directory relative which all output files are stored',
                    default='mimic3models/in_hospital_mortality/predictions/confvals')
    ...
    for i in range(args.n_iters):
        if args.stratify:
            predictions, labels = sk_utils.resample(data['prediction'], data['y_true_l'], n_samples=data.shape[0], replace=True, stratify=data['y_true_l'])
        else: 
    ...
    for dem in data[['binaryIns', 'Gender', 'Ethnicity']].columns:
        results[dem] = dict()
        
        for val in data[dem].unique():
